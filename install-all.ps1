...
$Skills = @(
    ...
    @{
        Name = "ecommerce-visual-strategy"
...
function Ensure-GhAuth {
    ...
    Write-Host "GitHub login is required for private skills. Complete the browser/device authorization that opens next."
...
function Install-SkillFromArchive {
    ...
    Download-RepoArchive -Repo $Skill.Repo -Branch $Skill.Branch -Private ([bool]$Skill.Private) -OutputZip $zipPath

    Expand-Archive -Path $zipPath -DestinationPath $tempRoot -Force
    $repoRoot = Get-ChildItem -Path $tempRoot -Directory | Select-Object -First 1
    if (-not $repoRoot) { throw "Downloaded archive did not contain repository files: $($Skill.Repo)" }

    if ($Skill.SkillPath -eq "." -or $Skill.SkillPath -eq "") {
        $sourceDir = $repoRoot.FullName
    } else {
        $sourceDir = Join-Path $repoRoot.FullName $Skill.SkillPath
    }
    if (-not (Test-Path (Join-Path $sourceDir "SKILL.md"))) {
        throw "SKILL.md not found in $($Skill.Repo)/$($Skill.SkillPath)"
    }
...
foreach ($skill in $Skills) {
    Install-SkillFromArchive -Skill $skill -CodexHome $resolvedCodexHome
}

Write-Host ""
Write-Host "All ecommerce skills installed. Restart Codex if they do not appear immediately."
